...
import { handleLoadingStatus } from '../../../utils/functions'
...
function  Arrivals({arrivals}) {
    ...
    return (
        ...
        arrivals.map((item=>(
            ...
            <TouchableOpacity style={{backgroundColor:"#F2F5FE",flexDirection:"row"}} key={item.id} onLongPress={()=>onShare(item,1)}>
                <Container>
                ...
                <Text numberOfLines={1} style={{flex:1,fontWeight:"bold",color:'#21305A',marginLeft:10,fontSize:12,alignSelf:'center'}}>{item.departureStationName}</Text>

                <TimeWrapper>
                <Feather name={'clock'} color={'#21305A'} size={DIMENS_UI_ICONS_SMALL} style={{alignSelf:'center'}} style={{marginRight:5}}></Feather>
                    <Text style={{fontSize:DIMENS_REGULAR,color:'#21305A',fontWeight:'bold'}}>{`${formatSchedule(item.arrivalTime)} - ${formatSchedule(item.departTime)}`}</Text>
                </TimeWrapper>
                </Container>
            </TouchableOpacity>
